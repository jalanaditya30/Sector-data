...
Research-only companion to the production V2 radar.  Frozen rules are taken from
V6.2 validation and MUST NOT be tuned by this live script.

R2 High Conviction
  persistence 80-100%, turnover expansion 1.7-2.5x, price 0..+5% vs prior 120D
  high, bull Midcap/Smallcap regime, strong industry group.

R5 Broader Candidate
  persistence >=60%, turnover expansion 1.7-2.5x, price -5..+5% vs prior 120D
  high, bull Midcap/Smallcap regime, strong industry group.

Every qualifying daily snapshot is archived once under early/v62_history/.  Existing
archive files are never rewritten.  Forward 20/40/60D outcomes are recalculated in
v62_prospective.json from those immutable signal snapshots.
"""
import json, os, sys
...
def fetch(tickers, period=PERIOD):
    ...
    for i in range(0,len(tickers),BATCH):
        ...
        z=yf.download(ch,period=period,interval='1d',group_by='ticker',auto_adjust=True,
...
def build_live():
    ...
    for t in uni:
        ...
        m=meta.get(t,{})
        rows.append({
          'ticker':t,'symbol':m.get('nse') or t.replace('.NS',''),'name':m.get('name') or t,
    ...
    secstat={k:{'median20':float(np.median(v)),'breadth20':float(np.mean(np.array(v)>0))}
             for k,v in sec.items() if v}

    candidates=[]
    for r in rows:
        s=secstat.get(r['sector'],{})
        sr=sector_regime(s.get('median20'),s.get('breadth20',0))
        f=r['f']; is_r2=(mr=='bull' and sr=='strong' and match(R2,f)); is_r5=(mr=='bull' and sr=='strong' and match(R5,f))
        if not is_r5:continue
        candidates.append({
          'ticker':r['ticker'],'symbol':r['symbol'],'name':r['name'],'sector':r['sector'],
          'bar_date':r['bar_date'],'entry_price':r['entry_price'],'candidate':'R2 High Conviction' if is_r2 else 'R5 Candidate',
          'r2':bool(is_r2),'r5':True,'turnover_persistence':round(f['turnover_persistence']*100,1),
          ...
          'r5_ret':round(f['r5'],2) if f['r5'] is not None else None,'r20':round(f['r20'],2) if f['r20'] is not None else None,
          'r60':round(f['r60'],2) if f['r60'] is not None else None,'market_regime':mr,'sector_regime':sr,
    ...
    payload={'generated':now,'model':'V6.2 Live Research Candidate','status':'research_candidate_not_buy_signal',
             ...
             'universe_requested':len(uni),'history_resolved':sum(t in frames for t in uni),
             'investable_rows':len(rows),'r2_count':sum(x['r2'] for x in candidates),'r5_total_count':len(candidates),
             'candidates':candidates,'notes':'R2/R5 thresholds are frozen from V6.2 validation. Liquidity is labelled, not optimized. V2 remains production radar.'}
    with open(os.path.join(HERE,'v62_live.json'),'w') as f:json.dump(payload,f,separators=(',',':'))

    # Immutable daily signal archive. Do not overwrite an existing day's first snapshot.
    ...
    if not os.path.exists(hp):
        ...
        print('archive exists; preserving immutable snapshot',hp,flush=True)
    return payload
...
def update_prospective():
    ...
    tickers=sorted({r['ticker'] for r in signals}|{MID,SMALL})
    frames=fetch(tickers,period='1y'); mid=frames.get(MID); small=frames.get(SMALL)
    evaluated=[]
    for r in signals:
    ...
    for label in ('R2 High Conviction','R5 Candidate','All R5'):
        rr=[x for x in evaluated if (label=='All R5' or x['candidate']==label)]
        summary[label]={}
        for h in HORIZONS:
...
def main():
    ...
    print(f"V6.2 live: market={p['market_regime']} R2={p['r2_count']} R5-total={p['r5_total_count']}",flush=True)
